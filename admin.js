...
function renderOrders() {
    ...
    if (!container) return; // Segurança REI NEO

    // Filtra para mostrar apenas pedidos que NÃO estão entregues (concluídos) 
    const activeOrders = localOrders.filter(o => o.status !== 'entregue');

    container.innerHTML = activeOrders.length ? '' : '<p>Nenhum pedido ativo no momento.</p>';
...
document.getElementById('btn-save-coupon').addEventListener('click', async () => {
    ...
    try {
        ...
        // REI NEO: Detalhando o erro para o log ficar mais claro
        const errorMsg = e.message || "Falha nas permissões do Supabase (RLS) ou conexão.";
        alert(`🚨 ERRO NO BANCO (CUPOM): ${errorMsg}`);
        showLoader(false);
    }
});
